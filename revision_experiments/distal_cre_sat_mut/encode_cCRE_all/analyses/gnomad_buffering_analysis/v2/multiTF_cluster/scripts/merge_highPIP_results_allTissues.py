#!/usr/bin/env python3
"""
Merge per-chromosome high-PIP shadow variant TSVs (all-tissues version) into a single file.
One row per (variant, cell_type, enhancer, lead_tissue).

Usage:
    python scripts/merge_highPIP_results_allTissues.py
"""
...
input_dir = 'results_final/shadow_per_chrom_allTissues'
output_path = 'results_final/highPIP_shadow_variants_allTissues.tsv'
...
#    Lead sign is looked up per (enhancer, cell_type, lead_tissue) since the lead
#    variant may differ across tissues.
lead_sign = (
...
print(f"  Unique tissues:   {merged['lead_tissue'].nunique():,}")

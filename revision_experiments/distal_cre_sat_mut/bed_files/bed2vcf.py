...
full_bed = pd.read_csv('GRCh38-dELS_only.bed', sep = '\t', header = None)
# make a dictionary for adding enhancer annotation to fasta tsv #
bed_dict = dict(zip([chrom + ':' + ('-').join([str(start), 
                                               str(stop)]) for chrom, start, stop in zip(full_bed[0],
                                                                                         full_bed[1],
                                                                                         full_bed[2])], # id to match tsv
                     full_bed[4]))
# open fasta file
dels_fasta_tsv = pd.read_csv('fasta_seqs/GRCh38-dELS_only.fasta', sep = '\t', header = None)
# add enhancer id #
...
all_vcf_dfs.to_csv('vcf_like/all_GRCh38-dELS_vcf_like.tsv', sep = '\t', index = False, header = False)

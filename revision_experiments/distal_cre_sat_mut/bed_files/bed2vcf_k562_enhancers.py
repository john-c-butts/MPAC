...
full_bed = pd.read_csv('K562.enhancers.bed', sep = '\t', header = None)
...
bed_dict = dict(zip([chrom + ':' + ('-').join([str(start), 
                     ...
                     full_bed[4]))
# open fasta file
dels_fasta_tsv = pd.read_csv('fasta_seqs/K562.enhancers.fasta', sep = '\t', header = None)
# add enhancer id #
...
all_vcf_dfs.to_csv('vcf_like/K562_enhancers_vcf_like.tsv', sep = '\t', index = False, header = False)
